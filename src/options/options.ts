import type { ExperienceControls, Schedule, ShortsMode, StoredState } from "../shared/model.js";
...
const experience = (): ExperienceControls => ({ disableAutoplay: checked("disable-autoplay"), hideShorts: checked("hide-shorts"),
  hideComments: checked("hide-comments"), hideLiveChat: checked("hide-live-chat"),
  hideRecommendations: checked("hide-recommendations"), hideHomeFeed: checked("hide-home-feed") });
const checked = (id: string): boolean => element<HTMLInputElement>(id).checked;
...
function render(state: StoredState, unlocked = false): void {
  ...
  for (const [id, key] of [["disable-autoplay", "disableAutoplay"], ["hide-shorts", "hideShorts"], ["hide-comments", "hideComments"],
    ["hide-live-chat", "hideLiveChat"], ["hide-recommendations", "hideRecommendations"], ["hide-home-feed", "hideHomeFeed"]] as const)
    element<HTMLInputElement>(id).checked = state.settings.experience[key];
}
...
element("save").addEventListener("click", () => void send({ type: "parent-mutation", mutation: { action: "save-settings", dailyLimitSeconds: minutes("daily-limit"), shortsMode: element<HTMLSelectElement>("shorts-mode").value as ShortsMode, shortsLimitSeconds: minutes("shorts-limit"), schedule: schedule(), experience: experience() } }));
